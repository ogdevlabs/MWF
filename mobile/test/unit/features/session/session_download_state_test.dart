...
void main() {
  ...
  group('SessionDownloadState derivation', () {
    ...
    test('returns inProgress when any exercise has in_progress manifest',
      ...
      // watchAllEntries() returns every row regardless of status — mirrors
      // how the Riverpod provider actually calls the DAO.
      final allEntries =
          await db.downloadManifestDao.watchAllEntries().first;

      final state = SessionDownloadState.derive(
        exerciseIds: exerciseIds,
        manifests: allEntries,
      );
